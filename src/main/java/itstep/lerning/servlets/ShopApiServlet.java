...
public class ShopApiServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if(name.length() < 3) {
            sendRest( resp, "error", "Min length of 'name' is 3 symbols!", null ) ;
            return ;
        }
        ...
        if(description.length() > 20) {
            sendRest( resp, "error", "Max length of 'description' is 20 symbols!", null ) ;
            return ;
        }

        String token = fields.get("token");
        if (token == null || token.isEmpty()) {
            sendRest(resp, "error", "Error 401, 403. ", null);
            return;
        }
